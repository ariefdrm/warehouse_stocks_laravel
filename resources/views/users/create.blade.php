<x-app-layout>
    ...
    <x-slot name="actions">
        <a href="{{ route('users.index') }}"
            class="flex items-center gap-2 px-4 py-2 text-sm font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition shadow-sm">
        ...
        <form action="{{ route('users.store') }}" method="POST"
            ...
            <div class="p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <x-text-input id="email" name="email" type="email" class="block w-full"
                            :value="old('email')" required placeholder="email@stokflow.com" />
                    ...
                    <div class="space-y-2">
                        ...
                        <x-text-input id="password_confirmation" name="password_confirmation" type="password"
                            class="block w-full" required placeholder="••••••••" />
                    </div>
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            @foreach ($roles as $role)
                                <label
                                    class="relative flex cursor-pointer rounded-2xl border bg-white p-4 shadow-sm focus:outline-none hover:border-blue-200 transition-all has-[:checked]:border-blue-600 has-[:checked]:bg-blue-50/50 group">
                                    <input type="radio" name="roles_id" value="{{ $role->id }}" class="sr-only"
                                        required {{ old('roles_id') == $role->id ? 'checked' : '' }}>
                                    <span class="flex flex-1">
                                        <span class="flex flex-col">
                                            <span
                                                class="block text-sm font-black text-slate-800 uppercase tracking-wider group-has-[:checked]:text-blue-700">{{ $role->role_name }}</span>
                                            <span class="mt-1 flex items-center text-xs text-slate-500 italic">
                                                Akses standar untuk {{ $role->role_name }}
                                            </span>
                                        </span>
                                    </span>
                                    <svg class="h-5 w-5 text-blue-600 hidden group-has-[:checked]:block"
                ...
                <div class="flex items-center justify-end gap-4 pt-6 border-t border-slate-100">
                    ...
                    <button type="submit"
                        class="px-10 py-3 bg-blue-600 hover:bg-blue-700 text-white font-black uppercase text-[11px] tracking-widest rounded-xl transition-all shadow-lg shadow-blue-200">
                        Buat Akun Sekarang
                    </button>
                </div>
            </div>
